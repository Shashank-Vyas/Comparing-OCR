...
# Load the image
image_path = 'Images\cframe268.jpg'
...
# OCR on the resized ROI using EasyOCR
reader = easyocr.Reader(['en'])
text_results = reader.readtext(resized_thresh)

# Extract the detected text from the OCR result
detected_text = " ".join([result[1] for result in text_results])
...
ws.append(["Frame Name", "Extracted Text"])

# Add the frame name and extracted text
frame_name = image_path.split("\\")[-1]  # Extract the frame name from the image path
ws.append([frame_name, detected_text])

# Save the workbook
wb.save(excel_filename)
print(f"Excel file '{excel_filename}' created and saved successfully.")
